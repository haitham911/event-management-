...
type eventServiceHandler struct {
	dbhandler    store.DatabaseHandler
	eventEmitter msgqueue.EventEmitter
}

func newEventHandler(databasehandler store.DatabaseHandler, eventEmitter msgqueue.EventEmitter) *eventServiceHandler {
...
func (eh *eventServiceHandler) newEventHandler(w http.ResponseWriter, r *http.Request) {
	...
	if nil != err {
		w.WriteHeader(500)
		fmt.Fprintf(w, "error occurred while decoding event data %s", err)
		return
	}
	id, err := eh.dbhandler.AddEvent(event)
	if nil != err {
		w.WriteHeader(500)
		fmt.Fprintf(w, "error occurred while persisting event %s", err)
		return
	}
